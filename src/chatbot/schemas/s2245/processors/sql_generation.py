"""SQL generation for 2245 - single-table budget structure"""
...
def create_sql_chain(sub_scheme_code: Optional[str] = None):
    ...
    if cached_prompt:
        ...
        if config:
            ...
            # Fallback context for 2245
            context = {
                'table_name': 'district_expenditure_2245',
                'table_sections': 'See TABLE_SECTIONS in config',
                'data_relationships': 'Single table structure with district + table_section_code',
                'common_patterns': 'Konkan districts, multi-year expenditure',
