"""Response generation for 2245 - formats budget/expenditure data with bilingual support"""
import json
...
def generate_response(question: str, results: Any) -> str:
    """
    Generate natural language response from SQL results for 2245.
    
    Logic:
    1. Format results with Indian currency notation (₹)
    2. Generate summary statistics for aggregations
    3. Handle bilingual context (Marathi + English)
    4. Use LLM to create conversational response
    
    Performance: O(n) for result formatting, ~500ms for LLM call
    """
    llm = _init_llm()
    ...
    if isinstance(results, str):
        ...
        if isinstance(results[0], dict):
            ...
            # Generate summary statistics for expenditure/budget columns
            try:
                numeric_cols = {}
                for result in results:
                    for key, value in result.items():
                        # Match expenditure and budget columns
                        if any(term in key.lower() for term in ['expenditure', 'budget', 'estimate', 'revised']):
                            try:
