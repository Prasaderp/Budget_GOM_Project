"""Response generation for 62450017 - formats loan/expenditure data with bilingual support"""
import json
...
def generate_response(question: str, results: Any) -> str:
    """
    Generate natural language response from SQL results for 62450017.
    
    Logic: Same as s2245 - format with Indian currency, summaries, bilingual context
    """
    llm = _init_llm()
    ...
    if isinstance(results, str):
        ...
        if isinstance(results[0], dict):
            # Limit results for prompt efficiency (max 20 rows, but 62450017 has only 5 anyway)
            ...
            try:
                ...
                for result in results:
                    for key, value in result.items():
                        ...
                        if any(term in key.lower() for term in ['expenditure', 'budget', 'estimate', 'grant', 'revised']):
                            try:
