"""SQL generation for 64010018 - single-table district expenditure structure"""
...
def create_sql_chain(sub_scheme_code: Optional[str] = None):
    """Create SQL generation chain with 64010018-specific single-table prompts"""
    llm = _init_llm()
    schema_info = get_schema_info()
    table_info = format_table_info_for_prompt(schema_info, sub_scheme_code)
    
    # Build prompt with caching - O(1) after first load
    cache_key = f"prompt_6401:{sub_scheme_code or 'default'}"
