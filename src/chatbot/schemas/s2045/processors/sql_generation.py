...
Q: Compare expenditure across all districts in Konkan
SQL: SELECT "district", "{exp_col}", "fiscal_year" FROM {de} WHERE "district" IN ('Mumbai City', 'Mumbai Suburban', 'Thane', 'Palghar', 'Raigad', 'Ratnagiri', 'Sindhudurg') AND "fiscal_year" = '{default_fy}' ORDER BY "{exp_col}" DESC LIMIT {{top_k}};

Q: Find districts with highest expenditure
SQL: SELECT "district", "{exp_col}", "fiscal_year" FROM {de} WHERE "fiscal_year" = '{default_fy}' AND "{exp_col}" > 0 ORDER BY "{exp_col}" DESC LIMIT {{top_k}};"""
