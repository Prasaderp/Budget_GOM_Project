"""2029-specific schema context generator - builds prompt context for 4-table structure"""
...
class SchemaContextGenerator:
    """Generates 2029 schema-specific context for prompts from BaseSchemeConfig"""
    
    @staticmethod
    def get_table_names(config: BaseSchemeConfig) -> Dict[str, str]:
        """Extract table names from scheme config forms (2029 structure)"""
        tables = {
            'budget_post_details': '',
            'post_status': '',
            'post_expenses': '',
    ...
    def generate_data_relationships(config: BaseSchemeConfig, table_names: Dict[str, str]) -> str:
        """Generate data relationships context from config (2029 structure)"""
        lines = []
    ...
    def generate_common_patterns(config: BaseSchemeConfig, custom_context: Optional[str] = None) -> str:
        """Generate common data patterns context from config (2029 structure)"""
        lines = []
    ...
    def generate_examples(config: BaseSchemeConfig, table_names: Dict[str, str], custom_examples: Optional[str] = None) -> str:
        """Generate example queries from config (2029 structure)"""
        if custom_examples:
    ...
    def generate_context(config: BaseSchemeConfig, custom_context: Optional[Dict[str, str]] = None) -> Dict[str, str]:
        """Generate complete context dictionary for 2029 prompts (CACHED)"""
        cache_key = f"s2029_{config.code}"
