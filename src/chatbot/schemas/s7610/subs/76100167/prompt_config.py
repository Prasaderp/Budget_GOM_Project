"""Subschema-specific prompt customizations for 76100167 - Public Health Program"""
...
PROMPT_CONFIG = PromptConfig(
    custom_context={
        'data_relationships': """- district_expenditure_76100167: District-wise public health program expenditure
  ...
  * COLUMNS:
    ...
    - "sub_scheme_code" (VARCHAR): '76100167'
...
- Current fiscal year for filtering: '2025-26'""",
        ...
        'examples': """Question: रायगडचा अर्थसंकल्पीय अंदाज
SQL Query: SELECT de."district", de."budget_estimate"
FROM district_expenditure_76100167 de
WHERE de."fiscal_year" = '2025-26' AND de."district" = 'Raigad'
LIMIT {top_k};

Question: एकूण सुधारित अंदाज
SQL Query: SELECT SUM(de."revised_estimate") as total_revised
FROM district_expenditure_76100167 de
WHERE de."fiscal_year" = '2025-26'
LIMIT {top_k};

Question: 2026-27 चा अर्थसंकल्पीय अंदाज
SQL Query: SELECT de."district", de."budget_estimate_2026_27"
FROM district_expenditure_76100167 de
WHERE de."fiscal_year" = '2025-26'
ORDER BY de."budget_estimate_2026_27" DESC
LIMIT {top_k};"""
    },
    table_names={
        'district_expenditure_table': 'district_expenditure_76100167'
    }
)
