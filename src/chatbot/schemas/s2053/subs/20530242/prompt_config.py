"""Subschema-specific prompt customizations for 20530242"""
...
- Designations: 'Divisional Officer' (use exact English name from schema)
...
- Divisions: Konkan Division = all 7 regular districts combined (excludes DCO Staff)""",
        "examples": """Question: What is the basic pay for Divisional Officer in Mumbai City?
SQL Query: SELECT bpd."basic_pay", bpd."designation", bpd."district", bpd."category"
FROM budget_post_details_20530242 bpd
WHERE bpd."district" = 'Mumbai City'
  AND bpd."designation" = 'Divisional Officer'
LIMIT {top_k};

Question: Show salary expenditure for Divisional Officer posts in Palghar in 2022-23
SQL Query: SELECT ue."district", ue."unit_account", ue."expenditure_2022_23"
FROM unit_expenditure_20530242 ue
WHERE ue."district" = 'Palghar'
  AND ue."unit_account" = '01- Salary'
LIMIT {top_k};

Question: Total sanctioned Divisional Officer posts in Thane district
...
Question: Medical expenses for Mumbai City for Divisional Officer cadre
SQL Query: SELECT pe."district",
       MAX(pe."medical_expenses") AS medical_expenses
FROM post_expenses_20530242 pe
WHERE pe."district" = 'Mumbai City'
GROUP BY pe."district"
LIMIT {top_k};""",
    },
    table_names={
        "budget_post_details_table": "budget_post_details_20530242",
        "post_status_table": "post_status_20530242",
        "post_expenses_table": "post_expenses_20530242",
        "unit_expenditure_table": "unit_expenditure_20530242",
    },
)
