...
router = APIRouter(prefix="/api/s64010018", tags=["API - 64010018"])
...
def list_district_expenditure(
    ...
    query = (
        ...
        .filter(
            DistrictExpenditure64010018.fiscal_year == fy,
            DistrictExpenditure64010018.sub_scheme_code == SUB_SCHEME_CODE,
            DistrictExpenditure64010018.district.in_(allowed_districts),
        )
        .order_by(DistrictExpenditure64010018.district)
        .offset(skip)
        .limit(limit)
    )
    return query.all()
...
def get_district_expenditure(
    ...
    item = (
        db.query(DistrictExpenditure64010018)
        .filter(
            DistrictExpenditure64010018.id == id,
            DistrictExpenditure64010018.sub_scheme_code == SUB_SCHEME_CODE,
        )
        .first()
    )
...
def create_district_expenditure(
    ...
    existing = (
        ...
        .filter(
            DistrictExpenditure64010018.fiscal_year == payload["fiscal_year"],
            DistrictExpenditure64010018.sub_scheme_code == SUB_SCHEME_CODE,
            DistrictExpenditure64010018.district == district,
            DistrictExpenditure64010018.taluka == DISTRICT_OFFICE,
        )
        .first()
    )
    if existing:
    ...
    item = create_row_family(db, DistrictExpenditure64010018, payload, request)
    db.commit()
    db.refresh(item)
...
def delete_district_expenditure(
    ...
    delete_row_family(db, DistrictExpenditure64010018, id, request)
    db.commit()
