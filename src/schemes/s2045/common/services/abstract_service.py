"""
Shared Abstract Data Service for s2045 sub-schemes.

This service consolidates duplicate logic across s2045 sub-schemes.
...
class SubSchemeAbstractService:
    """
    Generic abstract data service for s2045 sub-schemes.
    ...
    def get_district_abstract_data(
        ...
        Raises:
            ValueError: If district or sub_scheme_code is None or empty
        """
        if not district:
        ...
        try:
            # Query data for specific district with sub_scheme_code filter
            data_query = db.query(
    ...
    def get_district_abstract_charts_data(
        ...
        try:
            # Aggregated query with all metrics and sub_scheme_code filter
            data_query = db.query(
                self.model.unit_account,
    ...
    def get_all_districts_abstract_charts_data(
        ...
        try:
            # Aggregated query across all districts with sub_scheme_code filter
            data_query = db.query(
                self.model.district,
                func.sum(getattr(self.model, self.fy_field)).label('budget'),
                func.sum(getattr(self.model, self.expenditure_field)).label('expenditure'),
    ...
    def get_abstract_data(
        ...
        try:
            # Query all districts with sub_scheme_code filter
            data_query = db.query(
                self.model.unit_account,
                self.model.district,
                getattr(self.model, self.fy_field)
