...
async def ui_list_budget_details(
    ...
    if view == "summary":
        ...
        if labels:
            perm_posts = [
                int(
                    district_summary.get(d, {}).get("Permanent", {}).get("PostsCurr", 0)
                    or 0
                )
                for d in labels
            ]
            temp_posts = [
                int(
                    district_summary.get(d, {}).get("Temporary", {}).get("PostsCurr", 0)
                    or 0
                )
                for d in labels
            ]
        ...
        response = render(
            request,
            "schemes/s2045/subs/s20450091/budget_post_details_list.html",
            context,
        )
...
async def ui_edit_budget_detail_form(
    ...
    is_allowed, error_msg = validate_access_control(
    ...
    response = render(
        request,
        "schemes/s2045/subs/s20450091/budget_post_details_form.html",
        {
            "request": request,
            "districts": districts_for_filter,
            "categories": CATEGORIES,
...
async def ui_update_budget_detail(
    ...
    if SanctionedPostsCurr is not None:
        ...
        current_level_count = post_level_repo.get_count(
            id, sub_scheme, "budget_post_details_20450091", db_detail.fiscal_year
        )
