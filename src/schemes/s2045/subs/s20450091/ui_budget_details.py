"""UI routes for budget post details (Form D) - sub-scheme 20450091"""
...
router = APIRouter(prefix="/ui/s20450091/budget-post-details", tags=["UI - प्रपत्र ड"], include_in_schema=False)
...
async def ui_list_budget_details(
    ...
    if view == "summary":
        ...
        response = templates.TemplateResponse("schemes/s2045/subs/s20450091/budget_post_details_list.html", context)
...
async def ui_edit_budget_detail_form(request: Request, id: int, db: Session = Depends(get_db)):
    ...
    is_allowed, error_msg = validate_access_control(detail.district, auth_level, auth_unit, db)
    if not is_allowed:
    ...
    response = templates.TemplateResponse("schemes/s2045/subs/s20450091/budget_post_details_form.html", {
        "request": request,
        "districts": districts_for_filter,
        "categories": CATEGORIES,
        "classes": CLASSES_SHEET1_2,
        ...
        "da_percentage": float(da_percentage),
        "da_rate": da_rate,
        "api_base_path": "/ui/s20450091/budget-post-details/api/post-levels",
        "pay_matrix_api_path": "/ui/s20450091/budget-post-details/api/pay-matrix",
...
async def ui_update_budget_detail(
    ...
    if SanctionedPostsCurr is not None:
        ...
        current_level_count = post_level_repo.get_count(
            db_detail.id, sub_scheme, "budget_post_details_20450091", db_detail.fiscal_year
        )
    ...
    try:
        ...
        return templates.TemplateResponse("schemes/s2045/subs/s20450091/budget_post_details_form.html", {
            "request": request,
