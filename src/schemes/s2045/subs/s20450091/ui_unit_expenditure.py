"""UI routes for unit expenditure (Form A) - sub-scheme 20450091"""
...
router = APIRouter(prefix="/ui/s20450091/unit-expenditure", tags=["UI - प्रपत्र अ"], include_in_schema=False)
...
async def api_update_inline(
    ...
    AuditService.log_action(
        ...
        record_id=id,
        old_values=old_vals,
        new_values={k: getattr(record, k) for k in _INTERNAL_DATA_KEYS}
    )
    db.commit()
...
async def ui_list_unit_expenditure(
    ...
    if view == "summary":
        ...
        resp = templates.TemplateResponse("schemes/s2045/subs/s20450091/unit_expenditure_list.html", context)
...
async def ui_edit_unit_expenditure_form(request: Request, id: int, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("schemes/s2045/subs/s20450091/unit_expenditure_form.html", {
        "request": request,
        "districts": districts_for_filter,
        "primary_units": PRIMARY_UNITS,
        "item": item,
...
async def export_unit_expenditure_summary_excel(request: Request, db: Session = Depends(get_db)):
    ...
    headers_map = {
        ...
        "budget_curr_estimating_officer": "अर्थसंकल्पीय Curr प्राकक्लन",
        "budget_curr_controlling_officer": "अर्थसंकल्पीय Curr नियंत्रक",
        "budget_curr_admin_dept": "अर्थसंकल्पीय Curr प्रशासकीय",
        "budget_curr_finance_dept": "अर्थसंकल्पीय Curr वित्त",
    }
...
async def export_unit_expenditure_sheet_only(
    ...
    """Export only unit_expenditure sheet with throttling."""
    if not is_authenticated(request):
        raise HTTPException(status_code=401, detail="Not authenticated")
    auth_level = get_auth_level(request)
    auth_unit = get_auth_unit(request)
