"""UI routes for sub-scheme 22350338 district-wise expenditure."""
...
from .models import DistrictExpenditure22350338, SUB_SCHEME_CODE
...
router = APIRouter(
    prefix="/ui/s22350338/district-expenditure",
    tags=["UI - 22350338 सामाजिक सुरक्षा व कल्याण"],
...
async def ui_list_district_expenditure(
    ...
    query = (
        db.query(DistrictExpenditure22350338)
        .filter(
            DistrictExpenditure22350338.fiscal_year == fiscal_year,
            DistrictExpenditure22350338.sub_scheme_code == SUB_SCHEME_CODE,
        )
    )

    if district:
        query = query.filter(DistrictExpenditure22350338.district == district)
    else:
        query = query.filter(DistrictExpenditure22350338.district.in_(allowed_districts))

    total_count = query.with_entities(func.count(DistrictExpenditure22350338.id)).scalar()
    items = (
        query.order_by(DistrictExpenditure22350338.district)
    ...
    context = {
        ...
        "can_edit": can_edit,
        "resource_name": "22350338 जिल्हानिहाय खर्च",
    ...
    return render(request, 
        "schemes/s2235/subs/s22350338/district_expenditure_list.html",
        context,
    )
...
async def ui_edit_district_expenditure_form(
    ...
    item = resolve_editable_row(db, DistrictExpenditure22350338, id, request)
    ...
    context = {
        ...
        "item": item,
        "districts": allowed_districts,
        "resource_name": "22350338 जिल्हानिहाय खर्च संपादन",
        "districts_mr": DISTRICTS_MR,
        "auth_level": auth_level,
        "auth_role": auth_role,
        "fy_labels": fy_labels,
    ...
    return render(request, 
        "schemes/s2235/subs/s22350338/district_expenditure_form.html",
        context,
    )
...
async def ui_update_district_expenditure(
    ...
    db.flush()
    key_cols = natural_key_columns(DistrictExpenditure22350338)
    consolidate_row(db, DistrictExpenditure22350338, item.district, item.fiscal_year,
...
async def ui_division_total(
    ...
    # Calculate totals from all entries (including Divisional Commissioner)
    totals = (
        db.query(
            func.sum(DistrictExpenditure22350338.expenditure_prev3).label("total_exp_prev3"),
            func.sum(DistrictExpenditure22350338.expenditure_prev2).label("total_exp_prev2"),
            func.sum(DistrictExpenditure22350338.expenditure_prev1).label("total_exp_prev1"),
            func.sum(DistrictExpenditure22350338.budget_grant_curr).label("total_bg_curr"),
            func.sum(DistrictExpenditure22350338.revised_grant_curr).label("total_rg_curr"),
            func.sum(DistrictExpenditure22350338.budget_estimate_next).label("total_be_next"),
        )
        .filter(
            DistrictExpenditure22350338.fiscal_year == fiscal_year,
            DistrictExpenditure22350338.sub_scheme_code == SUB_SCHEME_CODE,
            DistrictExpenditure22350338.district.in_(KONKAN_DISTRICTS),
        )
        .first()
    )
    ...
    context = {
        ...
        "resource_name": "22350338 विभाग एकूण",
        "districts_mr": DISTRICTS_MR,
        "auth_level": auth_level,
        "auth_role": auth_role,
    ...
    return render(request, 
        "schemes/s2235/subs/s22350338/division_total.html",
        context,
    )
