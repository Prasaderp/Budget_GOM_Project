"""API routes for sub-scheme 22350338 - district-wise expenditure."""
...
from .models import DistrictExpenditure22350338, SCHEME_CODE, SUB_SCHEME_CODE
from .schemas import (
...
def list_district_expenditure(
    ...
    query = (
        ...
        .filter(
            DistrictExpenditure22350338.fiscal_year == fy,
            DistrictExpenditure22350338.sub_scheme_code == SUB_SCHEME_CODE,
            DistrictExpenditure22350338.district.in_(allowed_districts),
        )
        .order_by(DistrictExpenditure22350338.district)
        .offset(skip)
        .limit(limit)
    )
    return query.all()
...
def get_district_expenditure(
    ...
    item = (
        db.query(DistrictExpenditure22350338)
        .filter(
            DistrictExpenditure22350338.id == id,
            DistrictExpenditure22350338.sub_scheme_code == SUB_SCHEME_CODE,
        )
        .first()
    )
...
def create_district_expenditure(
    ...
    existing = (
        ...
        .filter(
            DistrictExpenditure22350338.fiscal_year == payload["fiscal_year"],
            DistrictExpenditure22350338.sub_scheme_code == SUB_SCHEME_CODE,
            DistrictExpenditure22350338.district == district,
            DistrictExpenditure22350338.taluka == DISTRICT_OFFICE,
        )
        .first()
    )
    if existing:
    ...
    item = create_row_family(db, DistrictExpenditure22350338, payload, request)
    db.commit()
    db.refresh(item)
...
def delete_district_expenditure(
    ...
    delete_row_family(db, DistrictExpenditure22350338, id, request)
    db.commit()
