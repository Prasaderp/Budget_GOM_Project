"""API routes for sub-scheme 22353408 - district-wise expenditure."""
...
router = APIRouter(prefix="/api/s22353408", tags=["API - 22353408 वृद्धांचे कल्याण"])
...
def get_district_expenditure(
    ...
    item = (
        db.query(DistrictExpenditure22353408)
        .filter(
            DistrictExpenditure22353408.id == id,
            DistrictExpenditure22353408.sub_scheme_code == SUB_SCHEME_CODE,
        )
        .first()
    )
...
def create_district_expenditure(
    ...
    existing = (
        ...
        .filter(
            DistrictExpenditure22353408.fiscal_year == payload["fiscal_year"],
            DistrictExpenditure22353408.sub_scheme_code == SUB_SCHEME_CODE,
            DistrictExpenditure22353408.district == district,
            DistrictExpenditure22353408.taluka == DISTRICT_OFFICE,
        )
        .first()
    )
    if existing:
    ...
    item = create_row_family(db, DistrictExpenditure22353408, payload, request)
    db.commit()
    db.refresh(item)
...
def delete_district_expenditure(
    ...
    delete_row_family(db, DistrictExpenditure22353408, id, request)
    db.commit()
