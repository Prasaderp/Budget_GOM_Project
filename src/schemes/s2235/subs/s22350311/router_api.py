...
def create_district_expenditure(
    ...
    existing = (
        ...
        .filter(
            ...
            DistrictExpenditure22350311.taluka == DISTRICT_OFFICE,
        )
        .first()
    )
    if existing:
    ...
    item = create_row_family(db, DistrictExpenditure22350311, payload, request)
    db.commit()
    db.refresh(item)
...
def delete_district_expenditure(
    ...
    delete_row_family(db, DistrictExpenditure22350311, id, request)
    db.commit()
