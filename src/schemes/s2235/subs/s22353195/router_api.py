"""API routes for sub-scheme 22353195 - district-wise expenditure."""
...
from .models import DistrictExpenditure22353195, SCHEME_CODE, SUB_SCHEME_CODE
from .schemas import (
...
router = APIRouter(prefix="/api/s22353195", tags=["API - 22353195 सामाजिक सुरक्षा व कल्याण"])
...
def list_district_expenditure(
    ...
    query = (
        ...
        .filter(
            DistrictExpenditure22353195.fiscal_year == fy,
            DistrictExpenditure22353195.sub_scheme_code == SUB_SCHEME_CODE,
            DistrictExpenditure22353195.district.in_(allowed_districts),
        )
        .order_by(DistrictExpenditure22353195.district)
        .offset(skip)
        .limit(limit)
    )
    return query.all()
...
def get_district_expenditure(
    ...
    item = (
        db.query(DistrictExpenditure22353195)
        .filter(
            DistrictExpenditure22353195.id == id,
            DistrictExpenditure22353195.sub_scheme_code == SUB_SCHEME_CODE,
        )
        .first()
    )
...
def create_district_expenditure(
    ...
    existing = (
        ...
        .filter(
            DistrictExpenditure22353195.fiscal_year == payload["fiscal_year"],
            DistrictExpenditure22353195.sub_scheme_code == SUB_SCHEME_CODE,
            DistrictExpenditure22353195.district == district,
            DistrictExpenditure22353195.taluka == DISTRICT_OFFICE,
        )
        .first()
    )
    if existing:
    ...
    item = create_row_family(db, DistrictExpenditure22353195, payload, request)
    db.commit()
    db.refresh(item)
...
def update_district_expenditure(
    ...
    item = resolve_editable_row(db, DistrictExpenditure22353195, id, request)
    ...
    db.flush()
    key_cols = natural_key_columns(DistrictExpenditure22353195)
    consolidate_row(db, DistrictExpenditure22353195, item.district, item.fiscal_year,
...
def delete_district_expenditure(
    ...
    delete_row_family(db, DistrictExpenditure22353195, id, request)
    db.commit()
