...
router = APIRouter(prefix="/api/s62450017", tags=["API - 62450017"])
...
def create_district_expenditure(
    ...
    existing = (
        ...
        .filter(
            ...
            DistrictExpenditure62450017.taluka == DISTRICT_OFFICE,
        )
        .first()
    )
    if existing:
    ...
    item = create_row_family(db, DistrictExpenditure62450017, payload, request)
    db.commit()
    db.refresh(item)
...
def update_district_expenditure(
    ...
    item = resolve_editable_row(db, DistrictExpenditure62450017, id, request)
    ...
    db.flush()
    key_cols = natural_key_columns(DistrictExpenditure62450017)
    consolidate_row(db, DistrictExpenditure62450017, item.district, item.fiscal_year,
...
def delete_district_expenditure(
    ...
    delete_row_family(db, DistrictExpenditure62450017, id, request)
    db.commit()
