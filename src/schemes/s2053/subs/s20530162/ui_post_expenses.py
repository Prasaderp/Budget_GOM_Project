...
async def api_update_inline(
    ...
    try:
        nps_unified_val = float(NPSUnified)
    except ValueError:
        return JSONResponse({"success": False, "message": "NPS value must be a valid number"}, status_code=400)
    ...
    sync_update = build_post_expenses_district_sync_update(
        ...
        other=Other,
        nps_unified=nps_unified_val,
    )
    if sync_update:
        db.query(PostExpenses).execution_options(**{TALUKA_SCOPE_ALL_OPTION: True}).filter(
    ...
    try:
        ...
        CacheService.invalidate_scheme_cache(scheme_code, record.fiscal_year)
    except Exception:
        pass
...
def get_post_expenses_charts_data(db: Session, fiscal_year: str = '2025-26', district: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "pie_expenses": {"labels": ["Medical", "Festival", "Swagram", "Other"], "values": [0, 0, 0, 0]},
...
async def ui_list_post_expenses(
    ...
    if view == "summary":
        ...
        response = render(request, "schemes/s2053/subs/s20530162/post_expenses_list.html", context)
...
async def ui_edit_post_expense_form(request: Request, id: int, db: Session = Depends(get_db)):
    ...
    return render(request, "schemes/s2053/subs/s20530162/post_expenses_form.html", {
        "request": request,
        "districts": districts_for_filter,
        "categories": CATEGORIES,
        "classes": CLASSES_SHEET3,
...
async def ui_update_post_expense(
    ...
    try:
        ...
        try:
            from src.schemes.s2053.subs.s20530028.shared.services.cache_service import CacheService
            scheme_code, _ = get_scheme_from_cookies(request)
            CacheService.invalidate_scheme_cache(scheme_code, db_item.fiscal_year)
        except Exception:
            pass
...
async def export_post_expenses_sheet_only(
    ...
    """Export only post_expenses sheet with throttling."""
    auth_level = get_auth_level(request)
    auth_unit = get_auth_unit(request)
    fiscal_year = get_fiscal_year_from_request(request, db)
    user_district = None
