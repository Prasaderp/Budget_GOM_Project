"""UI routes for post status (Form C) - sub-scheme 20530313"""
...
from .config import (
    ...
    CLASS_3_KEY, CLASS_4_KEY, VALID_CLASS_KEYS,
...
router = APIRouter(
    prefix="/ui/s20530313/post-status",
    tags=["UI - प्रपत्र क"],
...
def _prepare_chart_data(summary_data: Dict[str, Any], labels: list) -> Dict[str, Any]:
    """Prepare chart data from summary data with all required keys."""
    try:
...
async def export_post_status_summary_excel(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            CLASS_KEYS_ORDER = ['वर्ग-3', 'वर्ग-4', 'एकूण']
