"""UI routes for unit expenditure (Form A) - sub-scheme 20530313"""
...
router = APIRouter(prefix="/ui/s20530313/unit-expenditure", tags=["UI - प्रपत्र अ"], include_in_schema=False)
...
async def ui_list_unit_expenditure(
    ...
    if view == "summary":
        ...
        resp = render(request, "schemes/s2053/subs/s20530313/unit_expenditure_list.html", context)
...
async def ui_edit_unit_expenditure_form(request: Request, id: int, db: Session = Depends(get_db)):
    ...
    fiscal_year = get_fiscal_year_from_request(request, db)
    return render(request, "schemes/s2053/subs/s20530313/unit_expenditure_form.html", {
        "request": request,
        "districts": districts_for_filter,
        "primary_units": PRIMARY_UNITS,
        "item": item,
