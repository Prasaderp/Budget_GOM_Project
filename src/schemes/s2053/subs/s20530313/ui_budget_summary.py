...
from .config import POSITION_ORDER, CLASS_3_KEY, CLASS_4_KEY, VALID_CLASS_KEYS, HRA_RATE_MAP
...
router = APIRouter(
    prefix="/ui/s20530313/budget-summary",
...
TOTAL_CLASS_LABEL_MR = "वर्ग-3 व 4"
...
def _process_budget_query_results(query_results, internal_col_keys, da_rate: float, include_dearness: bool = True, include_hra: bool = True):
    ...
    for i, row in enumerate(query_results):
        ...
        if raw_class_value == CLASS_3_KEY: 
        ...
        processed_row = {
            ...
            "Approved Posts (Prev 1)": int(row.Sum_Sanctioned2425 or 0),
...
def get_budget_summary_data(db: Session, fiscal_year: Optional[str] = None, district: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        internal_col_keys = [
            "Approved Posts (Prev 1)", "Approved Posts (Curr)", "Special Pay", "Basic Pay", "Grade Pay",
...
async def ui_budget_summary_report(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        response = templates.TemplateResponse("schemes/s2053/subs/s20530313/budget_post_details_list.html", template_context)
...
async def download_budget_summary_excel(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        excel_col_order_detail = [
             "Sr No.", "Class", "Position", "Approved Posts (Prev 1)", "Approved Posts (Curr)",
