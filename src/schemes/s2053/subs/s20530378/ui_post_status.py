"""UI routes for post status (Form C) - sub-scheme 20530378"""
...
router = APIRouter(
    prefix="/ui/s20530378/post-status",
    tags=["UI - प्रपत्र क"],
...
async def ui_list_post_status(
    ...
    if view == "summary":
        ...
        response = render(request, "schemes/s2053/subs/s20530378/post_status_list.html", context)
...
async def ui_edit_post_status_form(request: Request, id: int, db: Session = Depends(get_db)):
    ...
    return render(request, "schemes/s2053/subs/s20530378/post_status_form.html", {
        "request": request,
        "districts": districts_for_filter,
        "categories": CATEGORIES,
        "classes": CLASSES_SHEET1_2,
