"""UI routes for budget post details (Form D) - sub-scheme 20530378"""
...
async def ui_list_budget_details(
    ...
    if view == "summary":
        ...
        response = render(request, "schemes/s2053/subs/s20530378/budget_post_details_list.html", context)
...
async def ui_edit_budget_detail_form(request: Request, id: int, db: Session = Depends(get_db)):
    ...
    relative_years = get_relative_fiscal_years(fiscal_year)
    response = render(request, "schemes/s2053/subs/s20530378/budget_post_details_form.html", {
        "request": request,
        "districts": districts_for_filter,
        "categories": CATEGORIES,
        "classes": CLASSES_SHEET1_2,
