"""UI routes for post expenses (Form B) - sub-scheme 20530233"""
...
router = APIRouter(
    prefix="/ui/s20530233/post-expenses",
    tags=["UI - प्रपत्र ब"],
...
async def ui_list_post_expenses(
    ...
    if view == "summary":
        ...
        response = render(request, "schemes/s2053/subs/s20530233/post_expenses_list.html", context)
...
async def ui_edit_post_expense_form(request: Request, id: int, db: Session = Depends(get_db)):
    ...
    return render(request, "schemes/s2053/subs/s20530233/post_expenses_form.html", {
        "request": request,
        "districts": districts_for_filter,
        "categories": CATEGORIES,
        "classes": CLASSES_SHEET3,
