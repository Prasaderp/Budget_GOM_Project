...
router = APIRouter(
    prefix="/ui/s20530233/budget-summary",
...
def _process_budget_query_results(query_results, internal_col_keys, prev1_label, curr_label, da_rate: float, include_dearness: bool = True, include_hra: bool = True):
...
def get_budget_summary_data(db: Session, fiscal_year: Optional[str] = None, district: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        processed_data = _process_budget_query_results(query_results, internal_col_keys, prev1_label, curr_label, da_rate, include_dearness, include_hra)
...
async def ui_budget_summary_report(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        response = templates.TemplateResponse("schemes/s2053/subs/s20530233/budget_post_details_list.html", template_context)
