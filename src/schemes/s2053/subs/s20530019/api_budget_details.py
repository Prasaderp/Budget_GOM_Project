...
from .ui_budget_details import _format_basic_pay
...
async def api_update_inline(
    ...
    current_level_count = post_level_repo.get_count(
        id, sub_scheme, "budget_post_details_20530019", fiscal_year
    )
    ...
    try:
        AuditService.log_edit(
            db, request, "budget_post_details", id, auth_user, old_values, new_values
        )
