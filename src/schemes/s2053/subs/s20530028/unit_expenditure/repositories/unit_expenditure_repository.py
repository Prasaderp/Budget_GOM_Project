...
class UnitExpenditureRepository:
    ...
    def get_by_filters(
        ...
        try:
            query = build_district_filter(
                self.db.query(UnitExpenditure), auth_level, auth_unit, UnitExpenditure
            ).filter(
            ...
            total_count = (
                query.with_entities(func.count(UnitExpenditure.id)).scalar() or 0
            )
            items = (
                query.order_by(UnitExpenditure.id)
    ...
    def get_primary_units(
        self, fiscal_year: str, sub_scheme_code: str, district: Optional[str] = None
    ) -> List[str]:
        """Get distinct primary units (unit_account) matching filters"""
        try:
            query = (
                self.db.query(UnitExpenditure.unit_account)
                .distinct()
                .filter(
                    UnitExpenditure.fiscal_year == fiscal_year,
                    UnitExpenditure.sub_scheme_code == sub_scheme_code,
    ...
    def get_record_data(
        self, fiscal_year: str, sub_scheme_code: str, district: str, primary_unit: str
    ) -> Optional[UnitExpenditure]:
    ...
    def get_summary_data(
        self, fiscal_year: str, district: Optional[str] = None, exclude_dco: bool = True
    ) -> List[Tuple]:
        """
        ...
        try:
            ...
            sum_exprs = [
                ...
                func.sum(UnitExpenditure.budget_curr_estimating_officer).label(
                    "budget_curr_estimating_officer"
                ),
                func.sum(UnitExpenditure.budget_curr_controlling_officer).label(
                    "budget_curr_controlling_officer"
                ),
                func.sum(UnitExpenditure.budget_curr_admin_dept).label(
                    "budget_curr_admin_dept"
                ),
                func.sum(UnitExpenditure.budget_curr_finance_dept).label(
            ...
            return (
                ...
                .filter(*base_filter)
                .group_by(UnitExpenditure.unit_account)
                .order_by(UnitExpenditure.unit_account)
                .all()
            )
        except Exception as e:
            raise ConnectionError(f"Database query failed: {str(e)}")
    ...
    def get_charts_data(
        ...
        try:
            ...
            return (
                ...
                .filter(*base_filter)
                .group_by(UnitExpenditure.district)
                .order_by(UnitExpenditure.district)
                .all()
            )
        except Exception as e:
            raise ConnectionError(f"Database query failed: {str(e)}")
