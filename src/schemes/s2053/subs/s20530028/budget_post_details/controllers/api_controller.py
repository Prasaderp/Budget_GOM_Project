...
async def api_get_pay_matrix_levels(
    stage: str, service: PayMatrixService = Depends(get_pay_matrix_service)
):
...
@router.get(
    "/api/da-rate", response_class=JSONResponse, dependencies=[Depends(verify_api_auth)]
)
...
async def api_update_inline(
    ...
    if not check_edit_permission_for_scheme(
        auth_role, auth_level, auth_unit, service.repository.db
    ):
    ...
    try:
        ...
        fiscal_year = get_fiscal_year_from_request(request, db)
        # C5b: levels reference the URL id (the caller's read-scope row),
        # never the write-redirected `record.id` -- a district/DCO caller's
        # office row carries no levels, so counting against it never fires.
        current_level_count = post_level_repo.get_count(
            id, sub_scheme, "budget_post_details_20530028", fiscal_year
        )
        ...
        # finds it (C2); levels_ref_id keeps the guard on the URL id (C5b).
        # update_inline() raises on any failure, so reaching this point
        # always means success -- there is no "success" key to gate on (it
        # never returns one), so consolidation must run unconditionally or
        # it silently never runs (docs/plan-taluka-remediation.md Phase 6).
        with writable_scope(record):
            result = service.update_inline(update_dto, sub_scheme, levels_ref_id=id)
        db.flush()
        consolidate_row(
            db,
            BudgetPostDetails,
