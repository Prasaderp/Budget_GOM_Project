...
from ..dto.budget_post_dto import (
    BudgetPostRecordDataDTO,
    BudgetPostUpdateDTO,
    BudgetPostFormUpdateDTO,
)
...
class BudgetPostService:
    ...
    def update_inline(
        ...
        sub_scheme_code: str,
        levels_ref_id: Optional[int] = None,
    ) -> Dict[str, Any]:
        """
        Update record via inline API

        `levels_ref_id` is the id `post_level_details` rows actually
        reference -- the URL id, not `update_dto.id` (which may be a
        write-redirected row for a district/DCO caller, docs/plan-taluka-
        remediation.md C5b). Defaults to `update_dto.id` so an unmigrated
        ...
        try:
            ...
            record.local_supplementary_allowance = (
                update_dto.local_supplementary_allowance
            )
    ...
    def update_form(
        ...
        `levels_ref_id` is the id `post_level_details` rows actually
        reference -- the URL id, not `record_id` (which may be a
        write-redirected row for a district/DCO caller, docs/plan-taluka-
        remediation.md C5b). Defaults to `record_id` so an unmigrated caller
