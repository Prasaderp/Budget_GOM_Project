...
class PostExpensesRepository:
    ...
    def get_classes_by_filters(
        ...
        try:
            query = (
                self.db.query(PostExpenses.class_type)
                .distinct()
                .filter(
    ...
    def get_by_filters(
        ...
        try:
            query = build_district_filter(
                self.db.query(PostExpenses), auth_level, auth_unit, PostExpenses
            ).filter(
                PostExpenses.fiscal_year == fiscal_year,
                PostExpenses.sub_scheme_code == sub_scheme_code,
            )
            ...
            items = (
                query.order_by(PostExpenses.id)
    ...
    def get_charts_district_data(
        ...
        try:
            ...
            return (
                query.group_by(PostExpenses.district)
                .order_by(PostExpenses.district)
                .all()
            )
        except Exception as e:
            raise ConnectionError(f"Database query failed: {str(e)}")
    ...
    def get_charts_class_district_data(
        ...
        try:
            ...
            return (
                query.group_by(PostExpenses.district, PostExpenses.class_type)
                .order_by(PostExpenses.district, PostExpenses.class_type)
                .all()
            )
        except Exception as e:
            raise ConnectionError(f"Database query failed: {str(e)}")
