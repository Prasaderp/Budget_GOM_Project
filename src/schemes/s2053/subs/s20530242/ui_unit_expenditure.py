...
async def ui_list_unit_expenditure(
    ...
    if view == "summary":
        ...
        resp = render(request, "schemes/s2053/subs/s20530242/unit_expenditure_list.html", context)
...
async def ui_edit_unit_expenditure_form(request: Request, id: int, db: Session = Depends(get_db)):
    ...
    fiscal_year = get_fiscal_year_from_request(request, db)
    return render(request, "schemes/s2053/subs/s20530242/unit_expenditure_form.html", {
        "request": request,
        "districts": districts_for_filter,
        "primary_units": PRIMARY_UNITS,
        "item": item,
...
async def export_unit_expenditure_summary_excel(request: Request, db: Session = Depends(get_db)):
    ...
    rel_years = get_relative_fiscal_years(fiscal_year)
    
    headers_map = {
        "SrNo": "अ. क्र.",
        "UnitAccount": "लेख्याची प्राथमिक आणि दुय्यम युनिट",
        "expenditure_prev4": f"प्रत्यक्ष रक्कमा {rel_years.get('fy_prev4', {}).get('short', '')}",
        "expenditure_prev3": f"प्रत्यक्ष रक्कमा {rel_years.get('fy_prev3', {}).get('short', '')}",
        "expenditure_prev2": f"प्रत्यक्ष रक्कमा {rel_years.get('fy_prev2', {}).get('short', '')}",
        "budget_prev1": f"अर्थसंकल्पीय अंदाज {rel_years.get('fy_prev1', {}).get('short', '')}",
        "forecast_prev1": f"सुधारीत अंदाज {rel_years.get('fy_prev1', {}).get('short', '')}",
        "budget_curr_estimating_officer": f"अर्थसंकल्पीय {rel_years.get('fy_curr', {}).get('short', '')} प्राकक्लन",
        "budget_curr_controlling_officer": f"अर्थसंकल्पीय {rel_years.get('fy_curr', {}).get('short', '')} नियंत्रक",
        "budget_curr_admin_dept": f"अर्थसंकल्पीय {rel_years.get('fy_curr', {}).get('short', '')} प्रशासकीय",
        "budget_curr_finance_dept": f"अर्थसंकल्पीय {rel_years.get('fy_curr', {}).get('short', '')} वित्त",
    }
