"""UI routes for post status (Form C) - sub-scheme 20530242"""
...
router = APIRouter(
    prefix="/ui/s20530242/post-status",
    tags=["UI - प्रपत्र क"],
...
def _prepare_chart_data(summary_data: Dict[str, Any], labels: list) -> Dict[str, Any]:
    """Prepare Plotly chart data structures from summary data.

    Mirrors PostStatusSummaryService.prepare_chart_data() from s20530028 as a
    standalone function for the monolithic file layout.

    Args:
        summary_data: Return value of get_post_status_summary_data()
        labels: District labels to chart (may be refined to dynamic list if all zeros)

    Returns:
        dict with chart data structures, or {} on error
    """
    try:
        ...
        if labels:
            chart_data['district_posts_by_status'] = {
                ...
                'भरलेली': dist_filled,
                'रिक्त': dist_vacant,
                'स्थायी': [int((dcmap.get(d, {}) or {}).get('Permanent', 0) or 0) for d in labels],
                'अस्थायी': [int((dcmap.get(d, {}) or {}).get('Temporary', 0) or 0) for d in labels],
            }
        ...
        logger.error(f"Error preparing chart data for Post Status (s20530242): {e}", exc_info=True)
        return {}
...
async def api_update_inline(
    ...
    consolidate_row(db, PostStatus, record.district, record.fiscal_year, {c: getattr(record, c) for c in key_cols})
    db.commit()
    try:
...
async def ui_list_post_status(
    ...
    if view == "summary":
        ...
        response = render(request, "schemes/s2053/subs/s20530242/post_status_list.html", context)
...
async def ui_edit_post_status_form(request: Request, id: int, db: Session = Depends(get_db)):
    ...
    return render(request, "schemes/s2053/subs/s20530242/post_status_form.html", {
        "request": request,
        "districts": districts_for_filter,
        "categories": CATEGORIES,
        "classes": CLASSES_SHEET1_2,
...
async def ui_update_post_status(
    ...
    try:
        ...
        from src.core.taluka.models import natural_key_columns
        key_cols = natural_key_columns(PostStatus)
        consolidate_row(db, PostStatus, db_item.district, db_item.fiscal_year, {c: getattr(db_item, c) for c in key_cols})
        db.commit()
        db.refresh(db_item)
        try:
            from src.routers.ui_taluka_selection import invalidate_district_status_cache
