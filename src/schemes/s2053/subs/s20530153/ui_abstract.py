...
router = APIRouter(
    prefix="/ui/s20530153/district-wise-abstract",
...
async def ui_district_wise_abstract(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        if auth_level == 'district':
            if not auth_unit:
                raise HTTPException(status_code=403, detail="District unit missing")
        ...
        elif auth_level == 'taluka':
            if not auth_unit:
                raise HTTPException(status_code=403, detail="Taluka unit missing")
    ...
    if pivot_df.empty:
        response = render(request, 
            "schemes/s2053/subs/s20530153/district_wise_abstract.html",
            {
                "request": request,
