"""UI routes for budget post details (Form D) - sub-scheme 20530153"""
...
async def ui_list_budget_details(
    ...
    if view == "summary":
        ...
        response = render(request, "schemes/s2053/subs/s20530153/budget_post_details_list.html", context)
...
async def ui_edit_budget_detail_form(request: Request, id: int, db: Session = Depends(get_db)):
    ...
    response = render(request, "schemes/s2053/subs/s20530153/budget_post_details_form.html", {
        "request": request,
        "districts": districts_for_filter,
        "categories": CATEGORIES,
        "classes": CLASSES_SHEET1_2,
        ...
        "da_rate": da_rate,
        "api_base_path": "/ui/s20530153/budget-post-details/api/post-levels",
        "pay_matrix_api_path": "/ui/s20530153/budget-post-details/api/pay-matrix",
        "sub_scheme_code": SUB_SCHEME_CODE,
        "table_name": "budget_post_details_20530153",
        "relative_years": get_relative_fiscal_years(fiscal_year)
    })
    response.headers.update(get_no_cache_headers())
    return response
    ...
    try:
        ...
        if SanctionedPostsCurr is not None:
            ...
            current_level_count = post_level_repo.get_count(
                id, sub_scheme, "budget_post_details_20530153", db_detail.fiscal_year
            )
            if SanctionedPostsCurr < current_level_count:
                raise HTTPException(status_code=400, detail=f"मंजूर पदे {SanctionedPostsCurr} ठेवता येत नाही कारण {current_level_count} स्तर आधीच आहेत. प्रथम स्तर हटवा.")
        ...
        consolidate_row(db, BudgetPostDetails, db_detail.district, db_detail.fiscal_year,
                         {c: getattr(db_detail, c) for c in key_cols})
        db.commit()
        ...
        return render(request, "schemes/s2053/subs/s20530153/budget_post_details_form.html", {
            "request": request,
            "error": e.detail if isinstance(e, HTTPException) and 400 <= e.status_code < 500 else "रेकॉर्ड अपडेट करण्यात अयशस्वी. कृपया पुन्हा प्रयत्न करा.",
            "districts": districts_for_filter,
            "categories": CATEGORIES,
            "classes": CLASSES_SHEET1_2,
            "designations": DESIGNATIONS,
