"""API controller for budget post details - sub-scheme 20530153"""
...
from src.utils_auth import get_auth_unit, get_auth_level, get_auth_role, get_auth_user, verify_api_auth

router = APIRouter(
    prefix="/ui/s20530153/budget-post-details",
    tags=["API - Budget Post Details 20530153"],
...
async def api_update_inline(
    ...
    current_level_count = post_level_repo.get_count(
        record.id, sub_scheme, "budget_post_details_20530153", record.fiscal_year
    )
    if SanctionedPostsCurr < current_level_count:
        return JSONResponse({"success": False, "message": f"मंजूर पदे {SanctionedPostsCurr} पेक्षा {current_level_count} स्तर आधीच अस्तित्वात आहेत. कृपया प्रथम स्तर हटवा."}, status_code=400)
