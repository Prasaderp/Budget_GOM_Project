...
router = APIRouter(
    prefix="/ui/s20530153/post-status",
    tags=["UI - प्रपत्र क"],
...
def get_post_status_summary_data(db: Session, fiscal_year: str = '2025-26', district: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        for row in query_results:
            ...
            class_key_mr = str(CLASS_MR_MAP.get(class_key, class_key))
            if category_str not in summary:
                summary[category_str] = {}
            if class_key_mr not in summary[category_str]:
                summary[category_str][class_key_mr] = {}
            if status_str not in summary[category_str][class_key_mr]:
                summary[category_str][class_key_mr][status_str] = {}
                
            target = summary[category_str][class_key_mr][status_str]
...
async def ui_list_post_status(
    ...
    if view == "summary":
        ...
        response = templates.TemplateResponse("schemes/s2053/subs/s20530153/post_status_list.html", context)
...
async def ui_edit_post_status_form(request: Request, id: int, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("schemes/s2053/subs/s20530153/post_status_form.html", {
        "request": request,
        "districts": districts_for_filter,
        "categories": CATEGORIES,
        "classes": CLASSES_SHEET1_2,
...
async def ui_update_post_status(
    ...
    try:
        old_values = {
            "posts": db_item.posts, "salary": db_item.salary, "grade_pay": db_item.grade_pay,
            "special_pay": db_item.special_pay, "dearness_allowance": db_item.dearness_allowance,
        ...
        try:
            ...
            logger.error(f"Failed to log audit action: {audit_err}")
