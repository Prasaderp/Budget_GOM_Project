"""UI routes for post status (Form C) - sub-scheme 20530304"""
...
router = APIRouter(
    prefix="/ui/s20530304/post-status",
    tags=["UI - प्रपत्र क"],
...
def _prepare_chart_data(summary_data: Dict[str, Any], labels: list) -> Dict[str, Any]:
    """Prepare chart data from summary data. Returns all 4 district_posts_by_status keys."""
    try:
...
async def ui_list_post_status(
    ...
    if view == "summary":
        ...
        response = render(request, "schemes/s2053/subs/s20530304/post_status_list.html", context)
...
async def ui_edit_post_status_form(request: Request, id: int, db: Session = Depends(get_db)):
    ...
    return render(request, "schemes/s2053/subs/s20530304/post_status_form.html", {
        "request": request,
        "districts": districts_for_filter,
        "categories": CATEGORIES,
        "classes": CLASSES_SHEET1_2,
...
async def ui_update_post_status(
    ...
    try:
        old_values = {
            ...
            "class_type": db_item.class_type, "status": db_item.status,
            "posts": db_item.posts, "salary": db_item.salary,
            "grade_pay": db_item.grade_pay, "special_pay": db_item.special_pay,
            "dearness_allowance": db_item.dearness_allowance,
            "local_supplementary_allowance": db_item.local_supplementary_allowance,
            "house_rent_allowance": db_item.house_rent_allowance,
            "travel_allowance": db_item.travel_allowance, "other": db_item.other
        }
        update_dict = {
