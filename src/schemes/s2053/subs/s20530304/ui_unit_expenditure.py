"""UI routes for unit expenditure (Form A) - sub-scheme 20530304"""
...
router = APIRouter(prefix="/ui/s20530304/unit-expenditure", tags=["UI - प्रपत्र अ"], include_in_schema=False)
...
async def ui_list_unit_expenditure(
    ...
    if view == "summary":
        ...
        resp = templates.TemplateResponse("schemes/s2053/subs/s20530304/unit_expenditure_list.html", context)
...
async def ui_edit_unit_expenditure_form(request: Request, id: int, db: Session = Depends(get_db)):
    ...
    fiscal_year = get_fiscal_year_from_request(request, db)
    relative_years = get_relative_fiscal_years(fiscal_year)
    return templates.TemplateResponse("schemes/s2053/subs/s20530304/unit_expenditure_form.html", {
        "request": request,
        "districts": districts_for_filter,
        "primary_units": PRIMARY_UNITS,
        "item": item,
...
async def ui_update_unit_expenditure(
    ...
    BudgetaryEstimatesCurrAdminDept: Optional[int] = Form(None),
    BudgetaryEstimatesCurrFinanceDept: Optional[int] = Form(None)
):
    auth_role = get_auth_role(request)
    auth_level = get_auth_level(request)
    ...
    try:
        ...
        if auth_level != 'district':
            ...
            if BudgetaryEstimatesCurrAdminDept is not None:
                db_item.budget_curr_admin_dept = BudgetaryEstimatesCurrAdminDept
