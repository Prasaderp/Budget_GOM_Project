"""Helper utilities for sub-scheme 76100149.
...
def ensure_fiscal_year_seeded(db: Session, fiscal_year: str) -> None:
    ...
    exists = (
        ...
        .filter(DistrictExpenditure76100149.fiscal_year == fiscal_year, DistrictExpenditure76100149.sub_scheme_code == SUB_SCHEME_CODE)
        .limit(1).first()
    )
    if exists:
        return
    rows = [
        DistrictExpenditure76100149(fiscal_year=fiscal_year, scheme_code=SCHEME_CODE, sub_scheme_code=SUB_SCHEME_CODE, district=d)
    ...
    for d in KONKAN_DISTRICTS:
        ensure_contribution_rows(db, DistrictExpenditure76100149, d, fiscal_year)
    db.commit()
