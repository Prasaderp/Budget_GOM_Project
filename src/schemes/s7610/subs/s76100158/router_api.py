"""API routes for sub-scheme 76100158 - district-wise expenditure."""
...
router = APIRouter(prefix="/api/s76100158", tags=["API - 76100158 जिल्हानिहाय खर्च"])
...
def list_district_expenditure(
    ...
    query = (
        ...
        .filter(
            DistrictExpenditure76100158.fiscal_year == fy,
            DistrictExpenditure76100158.sub_scheme_code == SUB_SCHEME_CODE,
            DistrictExpenditure76100158.district.in_(allowed_districts),
        )
        .order_by(DistrictExpenditure76100158.district)
        .offset(skip)
        .limit(limit)
    )
    return query.all()
...
def create_district_expenditure(
    ...
    existing = (
        ...
        .filter(
            DistrictExpenditure76100158.fiscal_year == payload["fiscal_year"],
            DistrictExpenditure76100158.sub_scheme_code == SUB_SCHEME_CODE,
            DistrictExpenditure76100158.district == district,
            DistrictExpenditure76100158.taluka == DISTRICT_OFFICE,
        )
        .first()
    )
    if existing:
    ...
    item = create_row_family(db, DistrictExpenditure76100158, payload, request)
    db.commit()
    db.refresh(item)
...
def update_district_expenditure(
    ...
    item = resolve_editable_row(db, DistrictExpenditure76100158, id, request)
        ...
        if district != item.district:
            ...
            existing = (
                ...
                .filter(
                    ...
                    DistrictExpenditure76100158.sub_scheme_code == SUB_SCHEME_CODE,
                    DistrictExpenditure76100158.district == district,
                    DistrictExpenditure76100158.taluka == item.taluka,
                )
                .first()
            )
            if existing:
    ...
    db.flush()
    key_cols = natural_key_columns(DistrictExpenditure76100158)
    consolidate_row(db, DistrictExpenditure76100158, item.district, item.fiscal_year,
...
def delete_district_expenditure(
    ...
    delete_row_family(db, DistrictExpenditure76100158, id, request)
    db.commit()
