"""API routes for sub-scheme 76100167 - district-wise expenditure."""
...
from .models import DistrictExpenditure76100167, SCHEME_CODE, SUB_SCHEME_CODE
from .schemas import (
...
def list_district_expenditure(
    ...
    query = (
        ...
        .filter(
            DistrictExpenditure76100167.fiscal_year == fy,
            DistrictExpenditure76100167.sub_scheme_code == SUB_SCHEME_CODE,
            DistrictExpenditure76100167.district.in_(allowed_districts),
        )
        .order_by(DistrictExpenditure76100167.district)
        .offset(skip)
        .limit(limit)
    )
    return query.all()
...
def create_district_expenditure(
    ...
    existing = (
        ...
        .filter(
            DistrictExpenditure76100167.fiscal_year == payload["fiscal_year"],
            DistrictExpenditure76100167.sub_scheme_code == SUB_SCHEME_CODE,
            DistrictExpenditure76100167.district == district,
            DistrictExpenditure76100167.taluka == DISTRICT_OFFICE,
        )
        .first()
    )
    if existing:
    ...
    item = create_row_family(db, DistrictExpenditure76100167, payload, request)
    db.commit()
    db.refresh(item)
        ...
        if district != item.district:
            ...
            existing = (
                ...
                .filter(
                    ...
                    DistrictExpenditure76100167.sub_scheme_code == SUB_SCHEME_CODE,
                    DistrictExpenditure76100167.district == district,
                    DistrictExpenditure76100167.taluka == item.taluka,
                )
                .first()
            )
            if existing:
...
def delete_district_expenditure(
    ...
    delete_row_family(db, DistrictExpenditure76100167, id, request)
    db.commit()
