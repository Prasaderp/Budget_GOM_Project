"""UI routes for sub-scheme 76100167 district-wise expenditure."""
...
from .models import DistrictExpenditure76100167, SUB_SCHEME_CODE
from .helpers import (
...
router = APIRouter(
    prefix="/ui/s76100167/district-expenditure",
    tags=["UI - 76100167 जिल्हानिहाय खर्च"],
...
async def ui_list_district_expenditure(
    ...
    query = (
        ...
        .filter(
            DistrictExpenditure76100167.fiscal_year == fiscal_year,
            DistrictExpenditure76100167.sub_scheme_code == SUB_SCHEME_CODE,
        )
    )

    if district:
        query = query.filter(DistrictExpenditure76100167.district == district)
    else:
        query = query.filter(DistrictExpenditure76100167.district.in_(allowed_districts))

    total_count = query.with_entities(func.count(DistrictExpenditure76100167.id)).scalar()
    items = (
        query.order_by(DistrictExpenditure76100167.district)
    ...
    context = {
        ...
        "can_edit": can_edit,
        "resource_name": "76100167 जिल्हानिहाय खर्च",
    ...
    return templates.TemplateResponse(
        "schemes/s7610/subs/s76100167/district_expenditure_list.html",
        context,
    )
...
async def ui_edit_district_expenditure_form(
    ...
    context = {
        ...
        "item": item,
        "districts": allowed_districts,
        "resource_name": "76100167 जिल्हानिहाय खर्च संपादन",
        "districts_mr": DISTRICTS_MR,
        "auth_level": auth_level,
        "auth_role": auth_role,
        "fy_labels": fy_labels,
    ...
    return templates.TemplateResponse(
        "schemes/s7610/subs/s76100167/district_expenditure_form.html",
        context,
    )
...
async def ui_update_district_expenditure(
    ...
    if district != item.district:
        existing = (
            db.query(DistrictExpenditure76100167)
            .filter(
                DistrictExpenditure76100167.fiscal_year == item.fiscal_year,
                DistrictExpenditure76100167.sub_scheme_code == SUB_SCHEME_CODE,
                DistrictExpenditure76100167.district == district,
            )
            .first()
        )
        if existing:
