"""Post expenses populator for sub-scheme 20290046.

8 districts, 4 class types per district, filled/vacant counts.
"""
...
def _write_district(ws, db: Session, model, district: str, row_map: Dict[str, int], fixed_row: int, fiscal_year: Optional[str]):
...
def _get_category_counts(db: Session, model, district: str, fiscal_year: Optional[str]) -> tuple:
...
def _write_fixed_row(ws, db: Session, model, district: str, fixed_row: int, fiscal_year: Optional[str]):
    ...
    values = [rep.medical_expenses, rep.festival_advance, rep.swagram_maharashtra_darshan, component_value, rep.other]
