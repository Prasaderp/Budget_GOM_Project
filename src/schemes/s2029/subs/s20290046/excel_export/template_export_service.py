"""Template-based Excel export service for sub-scheme 20290046.

Supports 7 districts + DCO Staff with production-grade throttling.
"""
import io
import os
...
def _get_processor_module(district: str):
    ...
    try:
        return __import__(
            f'src.schemes.s2029.subs.s20290046.excel_export.processors.{processor_name}',
...
def _apply_district_filter(wb: Workbook, district: str, only_sheet: Optional[str]) -> Workbook:
...
async def export_original_workbook_async(
    ...
    return await ExcelExportService.export_with_throttle(export_fn=generate, filename=base_filename, fiscal_year=fiscal_year)
...
def export_original_workbook(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to generate Excel: {e}")
    base_filename = "original_format" if only_sheet is None else f"{only_sheet}_original_format"
    return ExcelExportService.create_response(content=output, base_filename=base_filename, fiscal_year=fiscal_year)
