"""API controller for budget post details - sub-scheme 20290037"""
...
from .config import SCHEME_CONFIG, SUB_SCHEME_CODE, MARATHI_TO_ENGLISH_DESIGNATIONS
from .helpers import (
...
router = APIRouter(
    prefix="/ui/s20290037/budget-post-details",
    tags=["API - Budget Post Details 20290037"],
...
async def api_get_pay_matrix_levels(
    request: Request, stage: str, db: Session = Depends(get_db)
):
    """Get pay matrix levels for a stage"""
    if not is_authenticated(request):
        raise HTTPException(status_code=401, detail="Unauthorized")
    levels = (
        db.query(PayMatrix.level)
        .filter(PayMatrix.stage == stage)
        .order_by(PayMatrix.level)
        .all()
    )
...
async def api_get_pay_matrix_basic_pay(
    ...
    record = (
        db.query(PayMatrix)
        .filter(PayMatrix.stage == stage, PayMatrix.level == level)
        .first()
    )
    if not record:
...
async def api_get_designations(
    ...
    query = (
        db.query(BudgetPostDetails.designation)
        .distinct()
        .filter(
    ...
    designations = [
        row[0] for row in query.order_by(BudgetPostDetails.designation).all()
    ]
...
async def api_update_inline(
    ...
    level_count = PostLevelRepository(db).get_count(
        id, sub_scheme, "budget_post_details_20290037", record.fiscal_year
    )
    if SanctionedPostsCurr < level_count:
        return JSONResponse(
            {
                "success": False,
                "message": f"मंजूर पदे {SanctionedPostsCurr} पेक्षा {level_count} स्तर आधीच अस्तित्वात आहेत. कृपया प्रथम स्तर हटवा.",
            },
            status_code=400,
        )

    vals_int = [
        SanctionedPostsPrev1,
        SanctionedPostsCurr,
        SpecialPay,
        GradePay,
