...
async def ui_list_budget_details(
    ...
    if view == "summary":
        ...
        response = render(
            request,
            "schemes/s2029/subs/s20290037/budget_post_details_list.html",
            context,
        )
...
async def ui_edit_budget_detail_form(
    ...
    response = render(
        request,
        "schemes/s2029/subs/s20290037/budget_post_details_form.html",
        {
            "request": request,
            "districts": districts_for_filter,
            "categories": CATEGORIES,
    ...
    try:
        if SanctionedPostsCurr is not None:
            ...
            level_count = PostLevelRepository(db).get_count(
                id, sub_scheme, "budget_post_details_20290037", db_detail.fiscal_year
            )
            if SanctionedPostsCurr < level_count:
                raise HTTPException(
                    status_code=400,
                    detail=f"मंजूर पदे {SanctionedPostsCurr} ठेवता येत नाही कारण {level_count} स्तर आधीच आहेत. प्रथम स्तर हटवा.",
                )
