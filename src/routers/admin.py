...
async def admin_login(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        resp.set_cookie("admin_user", admin.username, httponly=True, samesite="lax",
                        max_age=86400, secure=_IS_PROD)
        return resp
...
async def admin_audit_dashboard(
    ...
    try:
        ...
        if username:
            safe = _escape_like(_sanitize(username))
            query = query.filter(models.AuditLog.username.ilike(f"%{safe}%"))

        total = query.count()
        logs = query.order_by(desc(models.AuditLog.timestamp)).offset((page - 1) * page_size).limit(page_size).all()

        summary = db.query(
            models.AuditLog.action, models.AuditLog.table_name,
            func.count(models.AuditLog.id).label('count')
        ).filter(models.AuditLog.timestamp >= cutoff).group_by(
            models.AuditLog.action, models.AuditLog.table_name
        ).all()

        user_activity = db.query(
            models.AuditLog.username, models.AuditLog.user_level,
            models.AuditLog.user_role, func.count(models.AuditLog.id).label('count')
        ).filter(models.AuditLog.timestamp >= cutoff).group_by(
